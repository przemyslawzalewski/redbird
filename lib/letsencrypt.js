...
var acmeStorage = undefined;

function init(certPath, port, logger, debug){
  ...
  logger && logger.info('Letsencrypt config %s', JSON.stringify(leStoreConfig, null, 2));

  acmeStorage = certPath;

  logger && logger.info('ACME storage set to %s', acmeStorage);

  // we need to proxy for example: 'example.com/.well-known/acme-challenge' -> 'localhost:port/example.com/'
  http.createServer(function (req, res){
    var uri = url.parse(req.url).pathname;
    var headerHost = req.headers.host;
    var headerHostname = req.hostname;
    var urlHostname =  url.parse(req.url).hostname;

    var hostname = urlHostname || headerHost || headerHostname;

    logger && logger.info('URL %s hostname %s req.headers.host %s headerHostname %s urlHostname %s',
      req.url, hostname, headerHost, headerHostname, urlHostname);

    var filename = path.join(certPath, hostname, '.well-known', 'acme-challenge', uri);
    ...
    logger && logger.info('LetsEncrypt CA trying to validate challenge %s from uri %s', filename, uri);
    ...
    fs.stat(filename, function(err, stats) {
      if (err || !stats.isFile()) {
        ...
        res.end();
        logger && logger.info(
          'LetsEncrypt CA validation failed with error %s',
          err ? JSON.stringify(err, null, 2) : 'Challenge file not found');
        return;
      }
 ...
 *  to avoid
 */
function getCertificates(domain, email, production, renew, logger, debug){
  // Storage Backend
  var leStore = require('le-store-certbot').create(leStoreConfig);

  // ACME Challenge Handlers
  var leChallenge = require('./le-challenge-fs').create({
    webrootPath: acmeStorage,
    debug: debug,
    logger
  });

  debug && logger && logger.info('Debug mode enabled');
  logger && logger.info('ACME Challenge storage %s', acmeStorage);
