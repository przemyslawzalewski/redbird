...
ReverseProxy.prototype.setupLetsencrypt = function (log, opts) {
  ...
  letsencrypt.init(opts.letsencrypt.path, letsencryptPort, log, debug);
...
ReverseProxy.prototype.updateCertificates = function (domain, email, production, renewWithin, renew, debug) {
  var _this = this;
  return letsencrypt.getCertificates(domain, email, production, renew, this.log, debug).then(function (certs) {
    if (certs) {
      var opts = {
        key: certs.privkey,
